  ...
  return (
    <div className="main-datail-container content-wrapper">
      ...
      <div className="disc-container">
        <div className="status">Status: <span>{grievance.status}</span></div>
...
{grievanceDetails && grievanceDetails.data && grievanceDetails.data.length > 0 && grievanceDetails.data.map((item,key)=>{
    return(
<div className="reply-card-container"key={key}>

  <div className="reply-user-date">
      <div className="username">{item.replied_by}</div>
      <div className="reply-date">{item.created_at.split("T")[0]}</div>
  </div>
  <div className="user-reply">
  {item.reply}
  </div>
  <div className="attachment-file">
      <div>Attachments Added
      <div>{item.attachment}</div>
        {/* <div className="download-icon"> <img src={grievanceDetails.mediaUrl + item.attachment}/></div> */}
      </div>

      <div className="attachment-container" onClick={() => downloadFile(item)}>
      <div className="download-icon"> <img src="./img/download.png"/></div>
      </div>
